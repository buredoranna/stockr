...
for (dayn in trade_day_index_seq) { 
	...
	if (any(trade_day_index_seq == dayn)) {
		...
		while (length(subset_meanvar_df$stock_n) < 3) {
			print("debug: subset_meanvar_df$stock_n < 3")
		...
		#picks <- pick_list[[1]]
		picks <- pick_list$chosen_tripplet
...
index_fund_percents <- (index_fund_df$AdjClose[trade_day_index_seq] /
                        index_fund_df$AdjClose[start_index]) - 1
...
#	par(new=T)
#	plot(portfolio_percent_deltas_tradeday, type="l", xlim=xrange, ylim=yrange)
