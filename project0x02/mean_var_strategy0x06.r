...
hard_copy <- T ## if true, output performance graph to jpeg
...
## this takes a correlation data frame, and returns three stocks which have the lowest 
## correlation between them (or, at least, two of them)
pick_three_low_cor <- function(cor_df) {
	## debugode
	cor_df <- cor_stock_price_df
	lowest_correlations <- data.frame()
	for (stock in colnames(cor_df)) {
	    ## only considering absolute values helps with ordering values
	    temp <- abs(cor_df[, stock])
	    temp <- temp[order(temp)]
	    
	    ## these two stocks have the lowest correlation
	    temp <- temp[c(1,2)]
	
	    lowest_correlations <- rbind(lowest_correlations,
	                                 data.frame(as.numeric(temp[1]), as.numeric(temp[2])))
	
	}   
	colnames(lowest_correlations) <- c("cor1", "cor2")
	rownames(lowest_correlations) <- rownames(cor_df)

	## lastedit
	
	s <- 0
	stock1 <- lowest_correlations
	while(length(rownames(stock1)) > 1 ) {
		s <- s + 1
		cor1_min <- lowest_correlations[s, "cor1"]
		cor2_min <- lowest_correlations[s, "cor2"]
		stock1 <- lowest_correlations[lowest_correlations$cor1 <= cor1_min &
                                      lowest_correlations$cor2 <= cor2_min, ]
	}
	
	#stock1 <- lowest_correlations[lowest_correlations$cor1 <= cor1_min &
	#                              lowest_correlations$cor2 <= cor2_min, ]
	
	stock1 <- rownames(stock1)
	stock2 <- cor_df[stock1, ]
	stock2 <- stock2[order(stock2)]
	stock3 <- names(stock2)[2]
	stock2 <- names(stock2)[1]
	picks <- c(stock1, stock2, stock3)
	if(length(picks) < 3) {
		print(picks)
		readline("+++ error: number of picks less than three")
	}
...
if (plotdata == T) {
	...
	plot(index_fund_percents, col="darkblue", xlim=xrange, ylim=yrange, type="l",
         main = paste("data path: ", datapath, ", index fund: ", index_fund_name, "\n",
                      ...
                      " over ", years, " years, ",
                      selection_dominance, " dominant", sep=""))
