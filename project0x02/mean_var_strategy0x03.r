...
if (datapath == "tech/") {
	...
	#held_tech_stock_symbols <- c("nvda", "adbe", "csco", "ebay")
	...
	for (s in held_tech_stockn) {
		tech_portfolio_percent_value <- tech_portfolio_percent_value +
	                                    mydata[[s]][trade_day_index_seq, "Open"]
	}
...
index_fund_percents <- (sp500_df$Open[trade_day_index_seq] / sp500_df$Open[start_index]) - 1
...
cat(paste("window: ", nday_meanvar_window, "\n", sep=""))
...
print(managed_summary)
        
cat(paste("index return: ", round(tail(index_fund_percents,1) * 100, 1), "%\n", sep=""))
