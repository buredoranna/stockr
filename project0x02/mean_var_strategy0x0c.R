...
for (dayn in trade_day_index_seq) { 
	...
	if (any(trade_day_index_seq == dayn)) {
		...
		## stock selection ##
        #####################
		## to visualize what this is doing set "animate_selection_algorithm" to TRUE
...
average_annual_fund_return    <- (fund_return + 1) ^ (1 / years) - 1
...
average_annaul_portfolio_return <- (portfolio_return + 1) ^ (1 / years) - 1
