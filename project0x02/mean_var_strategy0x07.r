...
## list that holds data frames.

## number of days in roll window
nday_roll_window <- 25

readdata <- T
rolldata <- T ## if F, rolling mean/var is calculated on the fly, otherwise it is rolled
...
## trade every n days
trade_every_ndays <- sample(1:20, 1)
trade_every_ndays <- 1
...
read_data_fn <- function(datapath) {
	import_list <- list()
	stock_symbols <- list.files(datapath)
    for (f in 1:length(stock_symbols)) {
        fullpath <- paste(datapath, stock_symbols[f], sep="")
        print(fullpath)
        import_list[[f]] <- read.csv(fullpath, header=T)
    }
	
	return_list <- list(stock_symbols, import_list)
	return(return_list)
}
...
###################################################
roll_data_fn <- function(stock_list, nday_roll_window) {
    roll_list <- list()
	...
	cat(paste("mydata length: ", length(stock_list), "\n"))
    for (stock_n in 1:length(stock_list)) {
        stock_rollmean <- c(NA_padding, rollmean(stock_list[[stock_n]]$AdjClose, nday_roll_window))
        stock_rollvar  <- c(NA_padding, rollapply(stock_list[[stock_n]]$AdjClose, nday_roll_window, var))
        roll_df <- data.frame(stock_list[[stock_n]]$Date,
                              stock_list[[stock_n]]$AdjClose,
                              stock_rollmean,
                              stock_rollvar)
        colnames(roll_df) <- c("Date", "AdjClose", "rollmean", "rollvar")
...
viz_selection_algorithm <- function(x, y, hline, vline) {
    plot(x, y, xlim=c(0,100), ylim=c(0,1), pch=20)
    abline(h = hline)
    abline(v = vline)
}
...
#gen_meanvar_df_old <- function(mydata_p, dayn_p) {
#	for (stock in 1:length(stock_symbols)) {
#		meanvar_indexes <- ((dayn_p - nday_meanvar_window - 1):dayn_p) - 1
#
#		stock_mean <- mean(unlist(mydata_p[[stock]][meanvar_indexes, "AdjClose"]))
#        stock_var <- var(unlist(mydata_p[[stock]][meanvar_indexes, "AdjClose"]))
#
#		stock_value <- mydata_p[[stock]][dayn_p, "AdjClose"]
#
...
gen_meanvar_df_new <- function(roll_list_p, dayn_p) {
	trade_day_stock_values <- unlist(lapply(roll_list_p, "[", dayn_p, 2))
	trade_day_rollmeans    <- unlist(lapply(roll_list_p, "[", dayn_p - 1, 3))
	trade_day_rollvars     <- unlist(lapply(roll_list_p, "[", dayn_p - 1, 4))
	date_indexes <- rep(dayn_p, length(stock_symbols))
	trade_day_dates <- rep(as.Date(roll_list[[1]][dayn_p, "Date"]), length(stock_symbols))

	my_meanvar_df <- data.frame(stock_number_seq,
                                stock_symbols,
                                trade_day_rollmeans,
                                trade_day_rollvars,
                                ...
                                date_indexes,
                                trade_day_dates)
	colnames(my_meanvar_df) <- c("stock_n", "symbol", "mean", "var", "value", "date_index", "date")
	return(my_meanvar_df)
}
...
for (dayn in trade_day_index_seq) { 
	...
	if (any(trade_day_index_seq == dayn)) {
		...
		stock_adjclose_column <- which(colnames(roll_list[[1]]) == "AdjClose")
		...
		#### pick method:
		## 1) stock1 = "highest mean"
		## 2) order stock1 correlation row (low to high)
		## 3) stock2 = 1st in the ordered row
		## 4) stock3 = 2nd in the ordered row
		...
		#buy_stocks <- data.frame()
		#stock_picks <- pick_three_low_cor(cor_stock_price_df)
		#stock1_df <- subset_meanvar_df[subset_meanvar_df$symbol == stock_picks[1], ]
		#stock2_df <- subset_meanvar_df[subset_meanvar_df$symbol == stock_picks[2], ]
		#stock3_df <- subset_meanvar_df[subset_meanvar_df$symbol == stock_picks[3], ]
		#buy_stocks <- rbind(buy_stocks, stock1_df, stock2_df, stock3_df)
	...
	if (length(held_stocks) == 0) {
        ...
        for (x in 1:(length(buy_stocks$stock_n))) {
			...
			xaction_stock_value <- roll_list[[xaction_stock_number]][dayn, "AdjClose"] ## 
...
cat(paste("std dev: ", round(index_stddev, 4), "\n"))
